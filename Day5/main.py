...
def change_user_state(input_char,selected_word,current_word_state):
    new_current_state=''
    for i in range(0,len(selected_word)):
        if(selected_word[i]==input_char):
            new_current_state+=input_char
        else:
            new_current_state+=current_word_state[i]
    return new_current_state



def char_in_word(input_char,selected_word,current_word_state,attempts_remaining):
    if input_char in selected_word:
        current_word_state=change_user_state(input_char,selected_word,current_word_state)
    else:
        attempts_remaining-=1
    return current_word_state,attempts_remaining   
def check_status(selected_word,current_word_state,attempts_remaining):
...
def game_play():
    ...
    while(True):
        ...
        current_word_state,attempts_remaining=char_in_word(input_char,selected_word,current_word_state,attempts_remaining)
        print_user_string(current_word_state,attempts_remaining)

        ended=check_status(selected_word,current_word_state,attempts_remaining)
        if(ended):
            break
